...
if(strlen($_SESSION['alogin'])=="")
    ...
    $sql = "SELECT 
                                        ...
                                        cd.ClassNameNumeric, 
                                        cd.Section, 
                                        COUNT(sd.StudentId) AS TotalStudents,
                                        SUM(CASE WHEN rd.Grades > 0 THEN 1 ELSE 0 END) * 100.0 / COUNT(rd.id) AS PercentageGradesGreaterThanZero
                                    FROM 
                                        classdata cd
                                    JOIN 
                                        facultycombinationdata fcd ON cd.id = fcd.ClassId
                                    JOIN 
                                        facultydata fd ON fd.id = fcd.FacultyId
                                    JOIN 
                                        resultdata rd ON rd.ClassId = cd.id
                                    JOIN 
                                        studentdata sd ON rd.StudentId = sd.StudentId
                                    WHERE 
...
if ($query->rowCount() > 0) {
    foreach ($results as $result) {
        $pass_percentage = $result->PercentageGradesGreaterThanZero;
        $chartData[] = [
            'ClassName' => $result->ClassNameNumeric . '-' . $result->Section,
                    ...
                    <div class="main-page">
                        <div class="container-fluid">
                            <div class="row page-title-div">
                                <div class="col-md-6">
                                    <h2 class="title">Class Wise Result</h2>
...
<?php $cnt=$cnt+1;}} ?>
    ...
    am4core.ready(function() {
        ...
        categoryAxis.title.text = "Class Name";
